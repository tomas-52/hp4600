...
//based on snoop for 300 dpi scan changed BunchOfConfig, SetRegisters, CCDSetup and Table0x00001000
//where possible removed duplicities of tables and commands
//the scanned file dimensions are 2552 x 3518 pixels, the final dimensions are 2549 x 3517 pixels
//conversion to 150 dpi resolution is added, the resulting file dimensions are 1274 x 1758 pixels
//as there is now independent code to scan at 150 dpi the corresponding parts are disabled
// you can reenable or remove them at your will !!!
...
//the third line gives the width and height of the picture in pixels (2549x3517) after all changes
...
//the sixth row gives the resolution per meter on peripherals (here 300 dpi, but zeroes are accepted)
...
const char tmp_file_header[] = {
	...
	0xf8, 0x09, 0x00, 0x00, 0xbe, 0x0d, 0x00, 0x00,
...
const char bmp_file_header[] = {
	0x42, 0x4d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x36, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00,
	0xf5, 0x09, 0x00, 0x00, 0xbd, 0x0d, 0x00, 0x00,
	0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00,
	0x23, 0x2e, 0x00, 0x00, 0x23, 0x2e, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
...
const char file_header_150[] = {
	0x42, 0x4d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x36, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00,
	0xfa, 0x04, 0x00, 0x00, 0xde, 0x06, 0x00, 0x00,
	0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00,
...
int CCDSetup(char *buf){
	...
	memcpy(buf, 
		...
		"\x68\x08\x69\x50\x6a\x01\x6b\x0c\x6d\x48\x6e\x94\x6f\x0e\x70\x50"
		"\x71\xbe\x72\x0e\x73\xe8\x74\x96\x75\x0e\x76\xf0\x77\xc0\x78\x0e"
		"\x79\x48\x7a\x2b\x7b\x0e\x7c\x50\x7d\x55\x7e\x0e\x7f\xe8\x80\x2d"
	, 0x0000040);
	ret = usb_control_msg(devh, USB_TYPE_VENDOR + USB_RECIP_DEVICE, 0x0000004, REGISTERS, 0x0000000, buf, 0x0000040, 1000);
	memcpy(buf, 
		"\x81\x0e\x82\xf0\x83\x57\x84\x0e\x85\x48\x86\xfd\x87\x0e\x88\x50"
		"\x89\x3c\x8a\x0f\x8b\xe8\x8c\xff\x8d\x0e\x8e\xf0\x8f\x3e\x90\x0f"
		"\x9a\x00\x9b\x15\x9d\x04\x9e\x00\x9f\x00\xa0\x0e\xa1\x00\xa2\x69"
		"\xa3\x0e\xa4\x00\xa5\xd2\xa6\x0e\xa7\xff\xa8\x68\xa9\x0e\xaa\xff"
	, 0x0000040);
	ret = usb_control_msg(devh, USB_TYPE_VENDOR + USB_RECIP_DEVICE, 0x0000004, REGISTERS, 0x0000000, buf, 0x0000040, 1000);
	memcpy(buf, 
		"\xab\xd1\xac\x0e\xad\xff\xae\x3a\xaf\x0f\xb0\x4b\xb1\x01\xb9\x57"
		"\xba\x0a\xcd\x00\xce\x0f\xd4\x00\xd5\x8e\xd6\xe3\xd7\x38\xec\x00"
		"\xed\x00\xee\xc0\xef\x00\xf0\x00\xf1\x00\xf2\x00\xf3\x30\xf4\x38"
		"\xf5\x00\xf6\x01\xf7\x0a\xf8\xaa\xf9\x00\xfa\x55\xfb\x00\xfc\x69"
	, 0x0000040);
	ret = usb_control_msg(devh, USB_TYPE_VENDOR + USB_RECIP_DEVICE, 0x0000004, REGISTERS, 0x0000000, buf, 0x0000040, 1000);
...
int bunchOfConfig(char *buf){
	...
	ret = usb_control_msg(devh, USB_TYPE_VENDOR + USB_RECIP_DEVICE, 0x0000004, REGISTERS, 0x0000000, buf, 0x0000040, 1000);
	memcpy(buf, 
		"\xee\x62\xef\x80\xf0\x62\xf1\x1c\xf2\x00\xf3\x0c\xf5\x10\xf6\x00"
		"\xf7\x03\xf8\x02\xf9\x0a\xfa\x37\xfb\x77\xfc\x00\xfd\x6c\xfe\x0b"
		"\xff\x50"
	, 0x0000022);
...
int main(int argc, char **argv) {
	...
	FILE * tmp;
	FILE * infile;
	unsigned char inbuf[7656];
	unsigned char inbuf3[7656];
	unsigned char inbuf4[7656];
	unsigned char line3[3828];
...
//	showtime();
	...
	memcpy(buf, "\x7c\x00\x7d\xbd\x7e\x03\x7f\x00", 0x0000008);
	ret = usb_control_msg(devh, USB_TYPE_VENDOR + USB_RECIP_DEVICE, 0x0000004, REGISTERS, 0x0000000, buf, 0x0000008, 1000);
	for(scanloop = 0; scanloop < 55; scanloop++) {
		memcpy(buf, "\x00\x7a\x07\x00", 0x0000004);
		ret = usb_control_msg(devh, USB_TYPE_VENDOR + USB_RECIP_DEVICE, 0x0000004, DMAREAD, 0x0000000, buf, 0x0000004, 1000);
		...
		ret = usb_bulk_read(devh, 0x00000082, buf, 0x000fc00, 4870);
		print_file(buf, ret, tmp);
		ret = usb_bulk_read(devh, 0x00000082, buf, 0x0009600, 4870);
		print_file(buf, ret, tmp);
	}
	...
	for(linenum = 3517; linenum > 0; linenum--){
		fseek(infile, (linenum*7656+54), SEEK_SET);
		fread(inbuf, 7656, 1, infile);
		for (i=7649; i >= 2; i--) {
			fprintf(myfile, "%c", inbuf[i]);
		}
	}
	...
	memcpy(inbuf, file_header_150, 54);
	for (i=0; i < 54; i++){
		fprintf(myfile, "%c", inbuf[i]);
	}
	for(linenum = 0; linenum < 3517; linenum+=2){
		fseek(infile, (linenum*7648+54), SEEK_SET);
		fread(inbuf3, 7648, 1, infile);
		fread(inbuf4, 7648, 1, infile);
		for(i = 0; i < 7648; i+=6){
